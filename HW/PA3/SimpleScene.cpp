...
/*****************************/
void idle() {
	if (!rotateOn || transMode == 'v')return;
	glPushMatrix();
	glLoadMatrixd(cow2wld.m);
	glRotated(2, rotx, roty, rotz);
	glGetDoublev(GL_MODELVIEW_MATRIX, cow2wld.m);
	glPopMatrix();
	glutPostRedisplay();
}
...
void onMouseDrag(int x, int y) {
	...
	/*********************************************************************************/
	x -= trans_oldX, y -= trans_oldY;
	trans_oldX += x, trans_oldY += y;
	if (rotateOn&&transMode == 'm')return;
	double t[3] = { 0,0,0 };
	if (transMode == 'm' || togDirection == 2)t[togDirection] = x / 25.0;
	else t[0] = x / 25.0, t[1] = y / 25.0;
	glPushMatrix();
	glLoadIdentity();
	if (transMode == 'v') {
		if (rotateOn) {
			glTranslated(cow2wld.m[12], cow2wld.m[13], cow2wld.m[14]);
			glRotated(x, wld2cam[cameraIndex].m[0], wld2cam[cameraIndex].m[4], wld2cam[cameraIndex].m[8]);
			glTranslated(-cow2wld.m[12], -cow2wld.m[13], -cow2wld.m[14]);
		}
		else {
			glMultMatrixd(cam2wld[cameraIndex].m);
			glTranslated(t[0], t[1], t[2]);
			glMultMatrixd(wld2cam[cameraIndex].m);
		}
	}
	glMultMatrixd(cow2wld.m);
	if (transMode == 'm')glTranslated(t[0], t[1], t[2]);
	glGetDoublev(GL_MODELVIEW_MATRIX, cow2wld.m);
	glPopMatrix();
	/*********************************************************************************/
...
void onKeyPress(unsigned char key, int x, int y) {
	...
	/*********************************************************************************/
	if (key == 'r' || (key == 'm'&&transMode == 'v')) {
		rotx = rand() % 100, roty = rand() % 100, rotz = rand() % 100;
		if (!rotx && !roty && !rotz)++rotx;
	}
	if (key == 'r')rotateOn = !rotateOn;
	if (key == 'm')transMode = 'm';
	if (key == 'v')transMode = 'v';
	if ('x' <= key && key <= 'z')togDirection = key - 'x', rotateOn = false;
	/*********************************************************************************/
